...
public class MongoDBJobStore implements JobStore, Constants {
	...
	public static final DBObject KEY_AND_GROUP_FIELDS = BasicDBObjectBuilder.start().append(KEY_GROUP, 1).append(KEY_NAME, 1).get();
	...
	public boolean removeJob(JobKey jobKey) throws JobPersistenceException {
		...
		try {
			if (find.hasNext()) {
				...
				this.triggerCollection.remove(new BasicDBObject(TRIGGER_JOB_ID, jobObj.get("_id")));
	...
	public JobDetail retrieveJob(JobKey jobKey) throws JobPersistenceException {
		...
		try {
			Class<Job> jobClass = (Class<Job>)this.getJobClassLoader().loadClass((String)dbObject.get(JOB_CLASS));

			JobBuilder builder = JobBuilder.newJob(jobClass).withIdentity((String)dbObject.get(JOB_KEY_NAME), (String)dbObject.get(JOB_KEY_GROUP)).withDescription((String)dbObject.get(JOB_DESCRIPTION));

			JobDataMap jobData = new JobDataMap();
			for (String key : dbObject.keySet()) {
				if (!key.equals(JOB_KEY_NAME) && !key.equals(JOB_KEY_GROUP) && !key.equals(JOB_CLASS) && !key.equals(JOB_DESCRIPTION) && !key.equals("_id")) {
	...
	public void storeTrigger(OperableTrigger newTrigger, boolean replaceExisting) throws ObjectAlreadyExistsException, JobPersistenceException {
		...
		DBObject dbObject = this.jobCollection.findOne(Keys.keyToDBObject(newTrigger.getJobKey()));
		if (dbObject != null) {
		...
		try {
			if (triggers.count() > 0) {
				...
				if (trigger.containsField(TRIGGER_JOB_ID)) {
					...
					// job.
					DBObject job = this.jobCollection.findOne(new BasicDBObject("_id", trigger.get(TRIGGER_JOB_ID)));
	...
	public OperableTrigger retrieveTrigger(TriggerKey triggerKey) {
		DBObject dbObject = this.triggerCollection.findOne(Keys.keyToDBObject(triggerKey));
		if (dbObject == null) {
			return null;
		}
	...
	public Set<JobKey> getJobKeys(GroupMatcher<JobKey> matcher) throws JobPersistenceException {
		...
		DBCursor cursor = this.jobCollection.find(this.queryHelper.matchingKeysConditionFor(matcher), KEY_AND_GROUP_FIELDS);
		try {
	...
	public Set<TriggerKey> getTriggerKeys(GroupMatcher<TriggerKey> matcher) throws JobPersistenceException {
		...
		DBCursor cursor = this.triggerCollection.find(this.queryHelper.matchingKeysConditionFor(matcher), KEY_AND_GROUP_FIELDS);
	...
	public List<String> getJobGroupNames() throws JobPersistenceException {
		return new ArrayList<>(this.jobCollection.distinct(KEY_GROUP));
	}


	@Override
	public List<String> getTriggerGroupNames() throws JobPersistenceException {
		return new ArrayList<>(this.triggerCollection.distinct(KEY_GROUP));
	}
	...
	public List<OperableTrigger> getTriggersForJob(JobKey jobKey) throws JobPersistenceException {
		...
		DBCursor cursor = this.triggerCollection.find(new BasicDBObject(TRIGGER_JOB_ID, dbObject.get("_id")));
		try {
			while (cursor.hasNext()) {
	...
	public void pauseTrigger(TriggerKey triggerKey) throws JobPersistenceException {
		this.triggerCollection.update(Keys.keyToDBObject(triggerKey), this.updateThatSetsTriggerStateTo(STATE_PAUSED));
	}


	@Override
	public Collection<String> pauseTriggers(GroupMatcher<TriggerKey> matcher) throws JobPersistenceException {
		final GroupHelper groupHelper = new GroupHelper(this.triggerCollection, this.queryHelper);
		this.triggerCollection.update(this.queryHelper.matchingKeysConditionFor(matcher), this.updateThatSetsTriggerStateTo(STATE_PAUSED), false, true);
	...
	public void resumeTrigger(TriggerKey triggerKey) throws JobPersistenceException {
		...
		this.triggerCollection.update(Keys.keyToDBObject(triggerKey), this.updateThatSetsTriggerStateTo(STATE_WAITING));
	}
	...
	public Collection<String> resumeTriggers(GroupMatcher<TriggerKey> matcher) throws JobPersistenceException {
		final GroupHelper groupHelper = new GroupHelper(this.triggerCollection, this.queryHelper);
		this.triggerCollection.update(this.queryHelper.matchingKeysConditionFor(matcher), this.updateThatSetsTriggerStateTo(STATE_WAITING), false, true);
	...
	public Set<String> getPausedTriggerGroups() throws JobPersistenceException {
		return new HashSet<>(this.pausedTriggerGroupsCollection.distinct(KEY_GROUP));
	}


	public Set<String> getPausedJobGroups() throws JobPersistenceException {
		return new HashSet<>(this.pausedJobGroupsCollection.distinct(KEY_GROUP));
	}
	...
	public void pauseAll() throws JobPersistenceException {
		final GroupHelper groupHelper = new GroupHelper(this.triggerCollection, this.queryHelper);
		this.triggerCollection.update(new BasicDBObject(), this.updateThatSetsTriggerStateTo(STATE_PAUSED));
	...
	public void resumeAll() throws JobPersistenceException {
		final GroupHelper groupHelper = new GroupHelper(this.triggerCollection, this.queryHelper);
		this.triggerCollection.update(new BasicDBObject(), this.updateThatSetsTriggerStateTo(STATE_WAITING));
	...
	public void pauseJob(JobKey jobKey) throws JobPersistenceException {
		final ObjectId jobId = (ObjectId)this.findJobDocumentByKey(jobKey).get("_id");
		final TriggerGroupHelper groupHelper = new TriggerGroupHelper(this.triggerCollection, this.queryHelper);
		List<String> groups = groupHelper.groupsForJobId(jobId);
		this.triggerCollection.update(new BasicDBObject(TRIGGER_JOB_ID, jobId), this.updateThatSetsTriggerStateTo(STATE_PAUSED));
	...
	public Collection<String> pauseJobs(GroupMatcher<JobKey> groupMatcher) throws JobPersistenceException {
		final TriggerGroupHelper groupHelper = new TriggerGroupHelper(this.triggerCollection, this.queryHelper);
		List<String> groups = groupHelper.groupsForJobIds(this.idsFrom(this.findJobDocumentsThatMatch(groupMatcher)));
		this.triggerCollection.update(this.queryHelper.inGroups(groups), this.updateThatSetsTriggerStateTo(STATE_PAUSED));
	...
	public void resumeJob(JobKey jobKey) throws JobPersistenceException {
		...
		// TODO: port blocking behavior and misfired triggers handling from
		// StdJDBCDelegate in Quartz
		this.triggerCollection.update(new BasicDBObject(TRIGGER_JOB_ID, jobId), this.updateThatSetsTriggerStateTo(STATE_WAITING));
	}
	...
	public Collection<String> resumeJobs(GroupMatcher<JobKey> groupMatcher) throws JobPersistenceException {
		final TriggerGroupHelper groupHelper = new TriggerGroupHelper(this.triggerCollection, this.queryHelper);
		List<String> groups = groupHelper.groupsForJobIds(this.idsFrom(this.findJobDocumentsThatMatch(groupMatcher)));
		this.triggerCollection.update(this.queryHelper.inGroups(groups), this.updateThatSetsTriggerStateTo(STATE_WAITING));
				...
				try {
					...
					try {
						...
						if (this.isTriggerLockExpired(existingLock)) {
							this.log.warn("Lock for trigger " + trigger.getKey() + " is expired - removing lock and retrying trigger acquisition");
	...
	private Mongo connectToMongoDB() throws SchedulerConfigException {
		MongoClientOptions options = MongoClientOptions.builder().writeConcern(WriteConcern.ACKNOWLEDGED).build();
	...
	protected OperableTrigger toTrigger(TriggerKey triggerKey, DBObject dbObject) {
		...
		try {
			Class<OperableTrigger> triggerClass = (Class<OperableTrigger>)this.getTriggerClassLoader().loadClass((String)dbObject.get(TRIGGER_CLASS));
			trigger = triggerClass.newInstance();
		}
		catch (ClassNotFoundException e) {
			throw new RuntimeException("Could not find trigger class " + (String)dbObject.get(TRIGGER_CLASS));
		}
		catch (Exception e) {
			throw new RuntimeException("Could not instantiate trigger class " + (String)dbObject.get(TRIGGER_CLASS));
		}
		...
		DBObject job = this.jobCollection.findOne(new BasicDBObject("_id", dbObject.get(TRIGGER_JOB_ID)));
		if (job != null) {
	...
	protected boolean applyMisfire(OperableTrigger trigger) throws JobPersistenceException {
		...
		this.signaler.notifyTriggerListenersMisfired((OperableTrigger)trigger.clone());
	...
	private void ensureIndexes() {
		...
		this.jobCollection.createIndex(keys, null, true);

		keys = new BasicDBObject();
		keys.put(KEY_NAME, 1);
		keys.put(KEY_GROUP, 1);
		this.triggerCollection.createIndex(keys, null, true);

		keys = new BasicDBObject();
		keys.put(LOCK_KEY_NAME, 1);
		keys.put(LOCK_KEY_GROUP, 1);
		this.locksCollection.createIndex(keys, null, true);
		// remove all locks for this instance on startup
		this.locksCollection.remove(new BasicDBObject(LOCK_INSTANCE_ID, this.instanceId));

		keys = new BasicDBObject();
		keys.put(CALENDAR_NAME, 1);
		this.calendarCollection.createIndex(keys, null, true);
	}
	...
	protected void storeTrigger(OperableTrigger newTrigger, ObjectId jobId, boolean replaceExisting) throws ObjectAlreadyExistsException {
		...
		try {
			...
			if (replaceExisting) {
				trigger.remove("_id");
				this.triggerCollection.update(keyToDBObject(newTrigger.getKey()), trigger);
			}
			else {
	...
	private void unmarkTriggerGroupsAsPaused(Collection<String> groups) {
		this.pausedTriggerGroupsCollection.remove(QueryBuilder.start(KEY_GROUP).in(groups).get());
	}
	...
	private void unmarkJobGroupsAsPaused(Collection<String> groups) {
		this.pausedJobGroupsCollection.remove(QueryBuilder.start(KEY_GROUP).in(groups).get());
	}
	...
	private Collection<DBObject> findJobDocumentsThatMatch(GroupMatcher<JobKey> matcher) {
		final GroupHelper groupHelper = new GroupHelper(this.jobCollection, this.queryHelper);
